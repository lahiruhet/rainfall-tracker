(function (root, factory) {
  ...
  const STATION_COORDINATE_SOURCE = "https://github.com/nuuuwan/lk_irrigation/blob/main/data/static/stations.json";
  const THAMBUTHTHEGAMA_COORDINATE = Object.freeze({
    latitude: 8 + 7 / 60 + 16 / 3600,
    longitude: 80 + 19 / 60 + 32 / 3600,
    source: "https://irrigation.gov.lk/web/images/Publications/Hydrologyical_Annual_2021-22.pdf",
    note: "Official Irrigation Department Hydrological Annual 2021/22, principal hydrometric stations table: 8°07′16″N, 80°19′32″E."
  });
  const DAILY_FIELDS = [
  ...
  function buildArchive({ mirrorRecords = [], stationMetadata = [], rivers = [], liveFeatures = [], start = ARCHIVE_START }) {
    ...
    for (const meta of stationMetadata) {
      ...
      const metadataLatitude = finite(meta.lat_lng?.[0]) ? Number(meta.lat_lng[0]) : null;
      const metadataLongitude = finite(meta.lat_lng?.[1]) ? Number(meta.lat_lng[1]) : null;
      const officialOverride = id === "thambuththegama" ? THAMBUTHTHEGAMA_COORDINATE : null;
      metaById[id] = {
        ...
        latitude: officialOverride?.latitude ?? metadataLatitude,
        longitude: officialOverride?.longitude ?? metadataLongitude,
        coordinateSource: officialOverride?.source || meta.coordinate_source ||
          (metadataLatitude !== null && metadataLongitude !== null ? STATION_COORDINATE_SOURCE : null),
        coordinateNote: officialOverride?.note || meta.coordinate_note || null
      };
    }
    ...
    for (const id of includedIds) {
      ...
      stationsById[id] = {
        ...
        coordinateSource: base.coordinateSource || null,
        coordinateNote: base.coordinateNote || null,
    ...
    for (const station of Object.values(stationsById)) {
      ...
      stations.push({
        ...
        coordinate_source: station.coordinateSource,
        ...(station.coordinateNote ? { coordinate_note: station.coordinateNote } : {}),
  ...
  return {
    ...
    STATION_COORDINATE_SOURCE, THAMBUTHTHEGAMA_COORDINATE,
